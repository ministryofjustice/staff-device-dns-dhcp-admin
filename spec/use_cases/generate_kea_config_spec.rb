...
describe UseCases::GenerateKeaConfig do
  describe "#call" do
    it "returns a default subnet used for smoke testing" do
      config = JSON.parse(UseCases::GenerateKeaConfig.new.call)
    ...
    it "appends subnets to the subnet4 list" do
      ...
      config = JSON.parse(UseCases::GenerateKeaConfig.new(subnets: [subnet1, subnet2]).call)
    ...
    it "includes global options in the config" do
      ...
      config = JSON.parse(UseCases::GenerateKeaConfig.new(subnets: [], global_option: global_option).call)
    ...
    it "does not set the global options if none are passed in" do
      config = JSON.parse(UseCases::GenerateKeaConfig.new(subnets: [], global_option: nil).call)
    ...
    it "does not set the valid lifetime for a subnet if the subnet option is not set" do
      ...
      config = JSON.parse(UseCases::GenerateKeaConfig.new(subnets: [subnet]).call)
    ...
    it "does not set the valid lifetime for a subnet if the subnet option does not set a valid lifetime" do
      ...
      config = JSON.parse(UseCases::GenerateKeaConfig.new(subnets: [option.subnet]).call)
    ...
    it "appends reservation to the subnet" do
      ...
      config = JSON.parse(UseCases::GenerateKeaConfig.new(subnets: [reservation.subnet]).call)
    ...
    it "appends multiple reservations to the subnet" do
      ...
      config = JSON.parse(UseCases::GenerateKeaConfig.new(subnets: [reservation1.subnet]).call)
    ...
    it "adds client class to the config when a client class is passed in" do
      ...
      config = JSON.parse(UseCases::GenerateKeaConfig.new(client_classes: [client_class, client_class2]).call)
    ...
    it "orders client classes correctly when subnet options are present" do
      ...
      config = JSON.parse(UseCases::GenerateKeaConfig.new(subnets: [subnet], client_classes: [client_class]).call)
    ...
    it "filters out nils from the client class array when subnets have no options" do
      ...
      config = JSON.parse(UseCases::GenerateKeaConfig.new(subnets: [subnet, subnet2]).call)
